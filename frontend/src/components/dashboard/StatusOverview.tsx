...
export function StatusOverview({ status }: StatusOverviewProps) {
  return (
    <div className="grid grid-cols-3 gap-4 md:grid-cols-4 lg:grid-cols-5">
      <MetricCard label="Files Tracked" value={status.files_tracked} />
      <MetricCard label="Files Mirrored" value={status.files_mirrored} />
      <MetricCard
        label="Pending Sync"
      ...
      />
      <MetricCard label="Drive Pairs" value={status.drive_pairs} />
      <MetricCard
