  ...
  return (
    <div className="flex flex-col h-full" data-testid="file-details">
      <div className="flex items-center justify-between border-b px-4 py-3">
        <h3 className="font-medium text-gray-900 dark:text-gray-100 truncate">{filename}</h3>
        <button
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-900 dark:text-gray-100">
            {drivePairName ?? `ID ${file.drive_pair_id}`}
          </p>
        </div>
        {file.checksum && (
          <div>
        ...
        {file.last_integrity_check_at && (
          <div>
            ...
            <p className="text-sm text-gray-900 dark:text-gray-100">
              {formatDate(file.last_integrity_check_at)}
            </p>
          </div>
        )}
        <div>
          ...
          <p className="text-sm text-gray-900 dark:text-gray-100">{formatDate(file.created_at)}</p>
        </div>
        <div>
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">
            Updated
          </p>
          <p className="text-sm text-gray-900 dark:text-gray-100">{formatDate(file.updated_at)}</p>
        </div>
      </div>
    </div>
  )
