...
const STATUS_CONFIG: Record<IntegrityStatus | 'internal_error', { label: string; color: string; icon: React.ReactNode }> = {
  ...
  both_corrupted: { label: 'Both corrupt', color: 'text-red-700 bg-red-50 border-red-200', icon: <XCircle className="h-4 w-4" /> },
  master_missing: { label: 'Primary missing', color: 'text-orange-700 bg-orange-50 border-orange-200', icon: <AlertTriangle className="h-4 w-4" /> },
...
export function IntegrityStatusBadge({ status, className }: IntegrityStatusBadgeProps) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-xs font-medium', cfg.color, className)}>
      {cfg.icon}
      {cfg.label}
    </span>
  )
