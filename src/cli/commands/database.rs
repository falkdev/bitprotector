...
pub fn handle(cmd: DatabaseCommand, repo: &Repository) -> anyhow::Result<()> {
    match cmd {
        ...
        DatabaseCommand::List => {
            ...
            println!("{:<6} {:<40} {:<10} {:<8} {:<20}", "ID", "Path", "MaxCopies", "Enabled", "LastBackup");
            println!("{}", "-".repeat(90));
            for cfg in &configs {
                println!(
                    "{:<6} {:<40} {:<10} {:<8} {}",
        ...
        DatabaseCommand::Show { id } => {
            ...
            println!("  Last Backup: {}", cfg.last_backup.as_deref().unwrap_or("(never)"));
        ...
        DatabaseCommand::Run(args) => {
            ...
            for r in &results {
                match r.status.as_str() {
                    "success" => println!("  [OK] Destination #{}: {}", r.backup_config_id, r.backup_path),
                    _ => println!("  [FAIL] Destination #{}: {}", r.backup_config_id, r.error.as_deref().unwrap_or("unknown error")),
                }
            }
