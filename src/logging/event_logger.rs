...
pub fn log_integrity_pass(
    ...
    log_event(
        ...
        "integrity_pass",
        Some(file_id),
        &format!("Integrity OK: {}", relative_path),
        None,
    )
}
...
pub fn log_integrity_fail(
    ...
    log_event(
        ...
        "integrity_fail",
        Some(file_id),
        &format!("Integrity FAIL: {}", relative_path),
        Some(status),
    )
}
...
pub fn log_sync_completed(repo: &Repository, file_id: i64, action: &str) -> anyhow::Result<()> {
    log_event(
        ...
        "sync_completed",
        Some(file_id),
        &format!("Sync action completed: {}", action),
        None,
    )
}
...
pub fn log_sync_failed(
    ...
    log_event(
        ...
        "sync_failed",
        Some(file_id),
        &format!("Sync action failed: {}", action),
        Some(error),
    )
}
