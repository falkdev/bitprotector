...
`sync_queue::process_all_pending` fetches a page of pending items together with the `drive_pair_id` of each item's tracked file (`Repository::list_pending_sync_queue_with_drive_pair`) and groups them by drive pair. It then spawns one worker thread per drive pair (via `std::thread::scope`) so that, for example, two large files queued on two different mirror drives copy at the same time instead of one full transfer blocking the other. Items belonging to the *same* drive pair are still processed strictly one at a time, in queue order, on that pair's single worker thread — this avoids two transfers contending for the same physical disk's I/O bandwidth. Each worker independently re-checks the `sync_settings.queue_paused` flag and the scheduler `stop_by` deadline before every item, and the outer loop re-checks both before fetching the next page, so pausing or hitting a time budget still halts new work promptly across all drive pairs.
