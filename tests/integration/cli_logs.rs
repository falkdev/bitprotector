use bitprotector_lib::cli::commands::logs::{handle, ListArgs, LogsCommand};
...
fn test_file_track_creates_log_entry() {
    ...
    let (entries, total) = repo
        .list_event_logs(Some("file_created"), None, None, None, 1, 50)
        .unwrap();
    assert_eq!(
        total, 1,
        "Tracking a file should create a file_created log entry"
    );
...
fn test_logs_show_entry() {
    let repo = make_repo();
    let entry = repo
        .create_event_log("integrity_pass", None, "test message", Some("details here"))
        .unwrap();
...
fn test_sync_operation_creates_log_entry() {
    use bitprotector_lib::core::{checksum, sync_queue};
    ...
    assert!(
        !entries.is_empty(),
        "A sync_completed event should be logged after processing"
    );
