...
use bitprotector_lib::cli::commands::database::{DatabaseCommand, AddArgs, RunArgs, handle};
...
fn test_backup_add_and_list() {
    ...
    handle(DatabaseCommand::Add(AddArgs {
...
fn test_backup_show() {
    let repo = make_repo();
    let dir = TempDir::new().unwrap();
    let cfg = repo.create_db_backup_config(dir.path().to_str().unwrap(), None, 5, true).unwrap();
...
fn test_backup_run_creates_file() {
    ...
    repo.create_db_backup_config(backup_dir.path().to_str().unwrap(), None, 5, true).unwrap();

    handle(DatabaseCommand::Run(RunArgs {
        db_path: db_file.path().to_str().unwrap().to_string(),
    }), &repo).unwrap();

    let entries: Vec<_> = std::fs::read_dir(backup_dir.path()).unwrap()
        .filter_map(|e| e.ok())
        .collect();
...
fn test_backup_rotation() {
    ...
    let cfg = repo.create_db_backup_config(backup_dir.path().to_str().unwrap(), None, 2, true).unwrap();
    ...
    let remaining: Vec<_> = std::fs::read_dir(backup_dir.path()).unwrap()
        .filter_map(|e| e.ok())
        .filter(|e| e.file_name().to_str().map(|n| n.ends_with(".db")).unwrap_or(false))
        .collect();
    assert_eq!(remaining.len(), 2, "Only max_copies=2 backups should remain");
}
