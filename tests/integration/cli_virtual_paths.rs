...
use bitprotector_lib::cli::commands::virtual_paths::{VirtualPathsCommand, SetArgs, RemoveArgs, RefreshArgs};
...
fn setup_tracked_file(repo: &Repository, primary: &TempDir, secondary: &TempDir, filename: &str) -> (bitprotector_lib::db::repository::DrivePair, bitprotector_lib::db::repository::TrackedFile) {
    let pair = repo.create_drive_pair("test_pair", primary.path().to_str().unwrap(), secondary.path().to_str().unwrap()).unwrap();
...
fn test_set_virtual_path_creates_symlink() {
    ...
    handle(VirtualPathsCommand::Set(SetArgs {
        file_id: file.id,
    ...
    assert!(symlink.is_symlink(), "Symlink should be created at docs/report.pdf");
...
fn test_remove_virtual_path_clears_symlink() {
    ...
    handle(VirtualPathsCommand::Remove(RemoveArgs {
        file_id: file.id,
        symlink_base: symlink_dir.path().to_str().unwrap().to_string(),
    }), &repo).unwrap();

    let symlink = symlink_dir.path().join("data/data.csv");
    assert!(!symlink.is_symlink(), "Symlink should be removed after remove command");
...
fn test_refresh_rebuilds_symlink_from_db_state() {
    ...
    repo.update_tracked_file_virtual_path(file.id, Some("/notes/notes.txt")).unwrap();

    handle(VirtualPathsCommand::Refresh(RefreshArgs {
        symlink_base: symlink_dir.path().to_str().unwrap().to_string(),
    }), &repo).unwrap();

    let symlink = symlink_dir.path().join("notes/notes.txt");
    assert!(symlink.is_symlink(), "Refresh should recreate missing symlinks");
}
...
fn test_remove_on_file_without_virtual_path_returns_error() {
    ...
    let result = handle(VirtualPathsCommand::Remove(RemoveArgs {
        file_id: file.id,
        symlink_base: symlink_dir.path().to_str().unwrap().to_string(),
    }), &repo);

    assert!(result.is_err(), "Remove on a file without a virtual path should return an error");
}
